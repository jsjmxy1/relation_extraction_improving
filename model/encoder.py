...
class PCNNEncoder(BaseModel):
    ...
    def __init__(
        ...
        self.self_attention = MultiHeadAttention(n_head = 8, d_model=256, d_k=32, d_v=32, dropout=0.1) 
    ...
    def forward(self, x, mask, is_train=True):
        ...
        att_x = self.self_attention(encoded_x, encoded_x, encoded_x, None)
        pooled_x = self._piecewise_pooling(att_x, mask)
        output_x = self.activation(pooled_x)
        output_x = self.dropout(output_x)
        return output_x
